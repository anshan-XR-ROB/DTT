...
class TriTraining(nn.Module):
    ...
    def classifier_voting(self, U_1, U_2, X, j, k, o, device_id):
        ...
        while True:
            U_batch_1 = U_1.next()
            U_imgs_1 = U_batch_1['data']
            batch_U_tensor_1 = U_imgs_1.cuda(non_blocking=True, device=device_id)

            U_batch_2 = U_2.next()
            U_imgs_2 = U_batch_2['data']
            batch_U_tensor_2 = U_imgs_2.cuda(non_blocking=True, device=device_id)

            mask_params = U_batch_1['mask_params']
            mask_params_tensor = mask_params.cuda(non_blocking=True, device=device_id)

            with torch.no_grad():
                _, pred_j_o_1 = self.classifiers[j](batch_U_tensor_1)
                _, pred_j_o_2 = self.classifiers[j](batch_U_tensor_2)
                _, pred_k_o_1 = self.classifiers[k](batch_U_tensor_1)
                _, pred_k_o_2 = self.classifiers[k](batch_U_tensor_2)

            pred_j_o = pred_j_o_1 * (1 - mask_params_tensor) + pred_j_o_2 * mask_params_tensor
            pred_k_o = pred_k_o_1 * (1 - mask_params_tensor) + pred_k_o_2 * mask_params_tensor
            ...
            if r > 0.9:
                ...
                batch_U_tensor = batch_U_tensor_1 * (1 - mask_params_tensor) + batch_U_tensor_2 * mask_params_tensor
...
U_1, U_sampler_1 = get_U_loader(VOC, \
                train_source=config.unsup_source, unsupervised=True, collate_fn=mask_collate_fn)
U_2, U_sampler_2 = get_U_loader(VOC, \
                train_source=config.unsup_source, unsupervised=True, collate_fn=mask_collate_fn)
X, X_sampler = get_U_loader(VOC, \
                train_source=config.train_source, unsupervised=False, collate_fn=collate_fn)
...
val_pre = ValPre()
val_dataset = VOC(data_setting, 'val', val_pre, training=False)
...
while improve:
    ...
    for i in range(3):    
        ...
        model.classifier_voting(U_loader_1, U_loader_2, X_loader, j, k, i, device_id)
