...
with Engine(custom_parser=parser) as engine:
    ...
    pretrained_dict = torch.load('/DTT/exp.voc/voc8.res50v3+.CPS+CutMix/ckpt/1-4-R101/epoch-last.pth')
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            _, ps_label_1_2 = torch.max(logits_cons_tea_1 + logits_cons_tea_2, dim=1)
            ps_label_1_2 = ps_label_1_2.long()

            # Get student#3 prediction for mixed image
            _, logits_cons_stu_3 = model(unsup_imgs_mixed, step=3)
            # Get student#2 prediction for mixed image
            # _, logits_cons_stu_2 = model(unsup_imgs_mixed, step=2)

            cps_loss = criterion(logits_cons_stu_3, ps_label_1_2)
            dist.all_reduce(cps_loss, dist.ReduceOp.SUM)
            cps_loss = cps_loss / engine.world_size
            cps_loss = cps_loss * config.cps_weight

            # supervised loss on both models
            _, sup_pred_l = model(imgs, step=3)
